...
describe 'compiled component fargate-v2' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/single_path_rules.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/single_path_rules/fargate-v2.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "TargetRule10" do
      ...
      it "to have property Conditions" do
          expect(resource["Properties"]["Conditions"]).to eq([{"Field"=>"path-pattern", "Values"=>["/v1"]}, {"Field"=>"host-header", "Values"=>["www.*"]}])
      end
