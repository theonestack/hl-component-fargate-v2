...
describe 'compiled component fargate-v2' do
  ...
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/service_discovery/fargate-v2.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "EcsFargateService" do
      ...
      it "to have property ServiceRegistries" do
          expect(resource["Properties"]["ServiceRegistries"]).to eq([{"RegistryArn"=>{"Fn::GetAtt"=>["ServiceRegistry", "Arn"]}, "ContainerName"=>"proxy"}])
      end
